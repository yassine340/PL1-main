...
export class CartService {
  ...
  public addToCart(productId: number, userId: number): Observable<Cart> {
    ...
    return this.http.post<Cart>(url, {});  // Ensure the backend handles the empty object
  }
  ...
  public updateCartQuantity(cartId: number, operation: string): Observable<void> {
    const url = `${this.baseUrl}/updateCartQuantite?action=${operation}&cartId=${cartId}`; // Corrected query params
    return this.http.get<void>(url);  // If using GET for updates, confirm with the backend, otherwise switch to PUT or PATCH
  }
